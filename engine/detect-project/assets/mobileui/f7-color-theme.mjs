...
function argbFromHex(hex2) {
  const isThree = 3 === (hex2 = hex2.replace("#", "")).length, isSix = 6 === hex2.length, isEight = 8 === hex2.length;
  if (!isThree && !isSix && !isEight) throw new Error("unexpected hex " + hex2);
  let r = 0, g = 0, b = 0;
  return isThree ? (r = parseIntHex(hex2.slice(0, 1).repeat(2)), g = parseIntHex(hex2.slice(1, 2).repeat(2)), b = parseIntHex(hex2.slice(2, 3).repeat(2))) : isSix ? (r = parseIntHex(hex2.slice(0, 2)), g = parseIntHex(hex2.slice(2, 4)), b = parseIntHex(hex2.slice(4, 6))) : isEight && (r = parseIntHex(hex2.slice(2, 4)), g = parseIntHex(hex2.slice(4, 6)), b = parseIntHex(hex2.slice(6, 8))), (255 << 24 | (255 & r) << 16 | (255 & g) << 8 | 255 & b) >>> 0;
}
...
`.trim();
function colorHexToRgb(hex2) {
  const h = hex2.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, (m, r, g, b) => r + r + g + g + b + b);
...
function colorThemeCSSProperties() {
  ...
  if (args.length === 1) {
    hex2 = args[0];
    rgb = colorHexToRgb(hex2);
  } else if (args.length === 3) {
    rgb = args;
    hex2 = colorRgbToHex(...rgb);
  }
  if (!rgb) return {};
  const {
    light,
...
// entry.mjs
function toCss(props) {
  const fmt = (v) => Array.isArray(v) ? v.join(", ") : v;
  const block = (sel, obj) => !obj || !Object.keys(obj).length ? "" : `${sel} {
${Object.entries(obj).map(([k, v]) => `  ${k}: ${fmt(v)};`).join("\n")}
}
`;
  return [
    block(":root", props.light),
    block(".ios", props.ios),
    block(".md", props.md),
    block(".dark", props.dark)
  ].join("");
}
var hex = process.argv[2];
if (!/^#[0-9a-fA-F]{6}$/.test(hex || "")) {
  console.error("usage: node f7-color-theme.mjs '#rrggbb'");
  process.exit(1);
}
console.log(`/* Framework7 theme palette generated from ${hex} by framework7's own`);
console.log(`   colorThemeCSSProperties() (via f7-color-theme.mjs). */`);
console.log(toCss(colorThemeCSSProperties(hex)));
